...
from time_tables.models import Subject, StudyRoom, TimeTable, Teacher, \
    TimeTableRecord, Student
from time_tables.tests.test_students import create_students, create_student
from time_tables.tests.test_study_rooms import create_study_rooms, \
    create_study_room
from time_tables.tests.test_subjects import create_subjects, create_subject
from time_tables.tests.test_teachers import create_teachers, create_teacher
from time_tables.tests.test_time_tables import create_time_tables, \
    create_time_table_records, create_time_table_record, create_time_table


def create_teachers_by_subjects(new_teachers_count, subject_multiple_count):
    """
    선생과 과목을 각각 생성하고, 과목별 선생을 추출
    :rtype: object
    :param new_teachers_count: 새로 생성되는 선생의 숫자를 받는 인수
    :param subject_multiple_count: 새로 생성되는 세 과목의 배수를 받는 인수
    :return: 
    """
    ...
    teachers = list(Teacher.objects
                    .filter(teacher_query)
...
def create_time_tables_by_study_rooms(new_time_tables_count,
                                      new_study_rooms_count):
    # time table N개 생성
    ...
    time_table_query = Q(id__gte=time_table_first_id)
    time_table_query &= Q(id__lte=time_table_last_id)
    time_tables = list(TimeTable.objects
                       .filter(time_table_query)
                       .values_list('name', flat=True))
    # study room M개 생성
    ...
    study_room_query = Q(id__gte=study_room_first_id)
    study_room_query &= Q(id__lte=study_room_last_id)
    study_rooms = list(StudyRoom.objects
                       .filter(study_room_query)
                       .values_list('name', flat=True))
    # 두 리스트 묶어서 추출
    item_list_time_tables_by_study_rooms = \
        {study_room: time_table
         for study_room, time_table in zip(study_rooms, time_tables)}
...
class TestScenarios(TestCase):
    ...
    def test_create_time_tables_by_study_rooms(self):
        ...
        self.assertEqual(time_table_by_study_room_ex1,
                         time_table_by_study_room_ex2)

    def test_connect_all_components(self):
        student: Student = create_student()
        subject: Subject = create_subject()
        study_room: StudyRoom = create_study_room()
        teacher: Teacher = create_teacher()
        time_table: TimeTable = create_time_table()
        time_table_record: TimeTableRecord = create_time_table_record()

        first = TimeTableRecord.objects.create(
        )
